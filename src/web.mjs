...
const server=http.createServer((req,res)=>{
  try {
    ...
    if(url.pathname.startsWith('/api/task/')){const id=url.pathname.slice('/api/task/'.length);const t=load(store,id);res.writeHead(200,{'content-type':'application/json; charset=utf-8'});res.end(JSON.stringify(t));return;}
    if(url.pathname!=='/'){res.writeHead(404);res.end('Not found');return;}
    const selected=url.searchParams.get('task');let task=null;if(selected)task=load(store,selected);
